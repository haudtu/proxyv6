...
install_3proxy() {
    ...
    wget https://raw.githubusercontent.com/xlandgroup/ipv4-ipv6-proxy/master/scripts/3proxy.service-Debian --output-document=/3proxy/3proxy-0.9.4/scripts/3proxy.service2
...
flush
auth strong
users s1teamdn:EpkGBHzVxqXRLWs
allow *
...
gen_proxy_file_for_user() {
    cat >proxy.txt <<EOF
$(awk -F "/" '{print $3 ":" $4 ":s1teamdn:EpkGBHzVxqXRLWs" }' ${WORKDATA})
EOF
}
...
upload_proxy() {
    ...
    echo "Proxy is ready! Format IP:PORT:USERNAME:PASSWORD"
